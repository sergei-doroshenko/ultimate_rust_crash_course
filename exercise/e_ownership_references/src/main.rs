...
fn main() {
    ...
    // This function consumes the argument by value.
    // That means that the variable is no longer useful.
    if eat(arg) {
       println!("Might be bananas");
    } else {
       println!("Not bananas");
    }
    // println!("After eating {}", arg); // Error: ^^^ value borrowed here after move
